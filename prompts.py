def _normalize_length(value: str) -> str:
    if not value:
        return "medium"
    v = value.strip().lower()
    if v in {"short", "small", "sm"}:
        return "small"
    if v in {"medium", "md"}:
        return "medium"
    if v in {"long", "large", "lg"}:
        return "large"
    return v  # fall back to whatever was passed
...
def build_instruction(opts: dict, require_json: bool = False) -> str:
    ...
    description_type = opts.get("description_type") or "Describe in Detail"
    tone = opts.get("tone") or "Friendly"
    # accept either output_length or length from client; normalize values
    raw_len = opts.get("output_length") or opts.get("length") or "Medium"
    output_length = _normalize_length(raw_len)
    # derive detail level dynamically if not provided
    raw_detail = opts.get("detail_level")
    detail_level = (raw_detail or ("simple" if output_length == "small" else "deep")).capitalize()
